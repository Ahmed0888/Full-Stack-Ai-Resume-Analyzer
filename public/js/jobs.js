...
//   if (!token) { jobsMsg.innerText = "Please login on index page"; return; }
//   try {
//     const res = await fetch("/api/jobs", { headers: { Authorization: "Bearer " + token } });
//     const data = await res.json();
//     if (!data.success) { jobsMsg.innerText = "Failed to load jobs"; return; }
...
//       </tr>
//     `).join("");
//   } catch (err) { console.error(err); jobsMsg.innerText = "Error fetching"; }
// }
...
//   const appliedDate = document.getElementById("jobDate").value;
...
//       body: JSON.stringify({ company, position, jobDescription, appliedDate, status, notes })
//     });
//     const data = await res.json();
//     if (!data.success) { jobsMsg.innerText = data.message || "Add failed"; return; }
//     jobsMsg.innerText = "Job added";
//     fetchJobs();
//   } catch (err) { console.error(err); jobsMsg.innerText = "Error adding job"; }
// });

// // Delegated edit/delete listeners
...
//     const row = e.target.closest("tr");
//     const id = row.dataset.id;
//     const newStatus = prompt("Enter new status (Applied/Interviewing/Offered/Rejected):", row.children[2].innerText);
//     if (!newStatus) return;
//     const token = getToken();
